...
Infrastructure owns provider-independent persistence records, aggregate record mapping, external-payload boundary validation, and the PostgreSQL adapter described below. The unknown-payload parser rejects unexpected keys and reconstructs fresh known primitive fields; semantic invariants remain in the domain. The application Clock owns both creation and consent timestamps. Source paths must begin with their matching locale and exclude dot segments, query strings, hashes, and unsafe encodings. The public Inquiry preparation form does not invoke this submission workflow; activation still requires abuse controls and notification integrations.

Inquiry infrastructure now implements the existing repository port with Drizzle and node-postgres. PostgreSQL stores the aggregate root and explicitly ordered Product snapshot rows under foreign-key, uniqueness, and closed-value checks. Save is transactional and never upserts. The adapter maps rows into provider-independent records and reconstitutes through the domain validation boundary. Composition obtains the repository lazily, so static rendering and presentation imports do not open database connections. SQL migrations are explicit deployment artifacts and never run during application startup.

The deployment direction is a private self-hosted server. The application reaches PostgreSQL over an internal Docker network; production PostgreSQL must not publish port 5432 publicly. Production, development, integration-test, future Umami, and future n8n databases use separate credentials and ownership boundaries.

First-party runtime containers use the fixed non-login UID/GID `10001:10001`, distinct from host operator identities. On the VPS, root owns deployment definitions, runtime settings, secret parents, locks, audit/operation logs, and release authority. The SSH operator remains outside the root-equivalent Docker group and may use passwordless sudo only for one root-owned, fail-closed deployment wrapper with fixed Compose/environment paths and a closed argument grammar. A root-owned standard-library Python helper strictly validates the authenticated release manifest, runtime schemas, and resolved Compose JSON before any operation. Monitoring activation remains root-only because cAdvisor receives Docker socket access. The wrapper exposes no arbitrary Docker, Compose, shell, systemd, secret, restore, retention-deletion, firewall, or database-administration capability.
