...
Automatic stale-record handling remains fail-closed. Exceptional SSH/root recovery may run only `yolpol-deploy reconcile-staging-pre-mutation <deployment-id>` from a direct root session; the wrapper rejects a non-root `SUDO_USER`, uses the existing global lock, and exposes neither environment/path selection nor generic ledger editing. The target must be the sole record for that canonical positive decimal ID and still be exactly `accepted`/`in-progress`/`never-started`, with null target migration fingerprint and result. The current policy-validated authority digest and migration fingerprint must exactly match its recorded previous state, its fixed journal path must not exist, and no other Staging database or deployment reconciliation blocker may exist. Success atomically records `reconciled-pre-mutation` plus terminal local failure `PRE_MUTATION_CRASH_RECONCILED`, preserving identity and historical GitHub synchronization state. It never reconciles post-authentication, journaled, migration, rollback, or manual-review ambiguity; never deletes journals or edits multiple blockers; and does not repair GitHub status. The operator and deployment-agent sudoers permissions remain unchanged.
...
Activation settings and credentials require a separate reviewed operation. The transaction has a fixed 75-minute deadline, but a process/host crash is not auto-resumed; stale in-progress ledger records fail closed unless a human root session proves every pre-mutation predicate and explicitly reconciles the one Staging record. Deployment-status retries depend on the still-valid encrypted job capability; local reconciliation neither requires nor fabricates one. Third-party workflow Actions still use mutable major-version tags; full commit-SHA pinning remains a required before-activation supply-chain hardening item after the exact SHAs are independently verified. There is no image/manifest signature layer, automatic database recovery, off-server durability proof, Production public cutover, or disaster-recovery proof in Phase C1.
