...
For iPhone/Safari development, serve Next.js on the LAN interface and open `http://192.168.1.55:3000`. That exact Origin is explicitly approved in `src/shared/config/inquiry-development.ts` and accepted only when `NODE_ENV=development`; other LAN hosts, ports, and schemes remain rejected. No development-origin environment variable is required. If the approved LAN address changes, update that typed configuration so `next.config.ts` and the Inquiry HTTP composition continue to share one exact value.
