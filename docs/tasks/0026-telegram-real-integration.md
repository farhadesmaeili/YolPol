...
Notifications are plain text with no `parse_mode`. They include the inquiry reference, customer/company/location/destination, preferred contacts, requested quantities, customer message, source locale, and the trusted `siteConfig.url` link `/en/staff/inquiries/<inquiryId>`. A subsequent non-Persian Customer Website message is rendered with its authored body and the succeeded Persian staff-target translation. If that translation is still `PENDING` or `RUNNING`, the worker defers the same outbox event with its existing bounded backoff before snapshotting or claiming recipients. The wait is bounded to 15 minutes from the durable event occurrence time; retry scheduling is capped at that deadline. A terminal `FAILED`/`CANCELLED` translation, or a translation still unresolved at the deadline, renders an explicit original-only fallback so notification delivery cannot remain blocked by translation. A Persian customer message has no redundant Persian-to-Persian translation row or notification section. Directional/control characters are neutralized and continuation lines are indented so customer content remains visibly subordinate. Text is capped at 3,900 UTF-16 units with code-point-safe per-section truncation while preserving both message sections when present, the routing reference, and Staff link. Internal pricing, access tokens, sessions, secrets, and database configuration are excluded.

`pnpm worker:inquiry-notifications` opens its own bounded PostgreSQL pool, loads only `DATABASE_URL` and `TELEGRAM_BOT_TOKEN`, composes the outbox/delivery/conversation repositories and provider, processes a bounded batch non-interactively, prints aggregate safe counters, closes the pool, and exits non-zero for pending retry, permanent failure, ambiguity, or process failure. Translation execution remains owned by the independent Conversation translation worker; the notification worker only reads its durable state. It does not invoke Email. This process is ready to become a separate service later; no Compose production service or secret is baked in now.
