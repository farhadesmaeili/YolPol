...
describe("production Export Logistics Product boundary", () => {
  it("returns exactly five verified eligible profiles and four unavailable clear Products", async () => {
    ...
    expect(model.unavailable.map(({id, name}) => ({id, name}))).toEqual([
      {id: "ylp-gb-250-cl-rd", name: "250ml Clear Round Glass Bottle"},
      {id: "ylp-gb-250-cl-sq", name: "250ml Clear Square Glass Bottle"},
      {id: "ylp-gb-500-cl-rd", name: "500ml Clear Round Glass Bottle"},
      {id: "ylp-gb-500-cl-sq", name: "500ml Clear Square Glass Bottle"},
    ]);
  });
});
