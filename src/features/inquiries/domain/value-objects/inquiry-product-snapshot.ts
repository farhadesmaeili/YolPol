...
export function createInquiryProductSnapshot(input: InquiryProductSnapshotInput): InquiryProductSnapshot {
  if (typeof input.productId !== "string" || !productIdPattern.test(input.productId)) throw new InquiryValidationError("items.productId", "Invalid Product snapshot ID.");
  ...
  return Object.freeze({productId: input.productId, sku: input.sku, slug: input.slug, productName: input.productName});
}
