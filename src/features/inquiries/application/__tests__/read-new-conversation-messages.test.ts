...
import {toConversationMessageDto, toStaffConversationMessageDto} from "@/features/inquiries/application/mappers/conversation-message-dto-mapper";
...
describe("ReadNewConversationMessages", () => {
  it("reads one bounded ordered page after the supplied cursor", async () => {
    ...
    const result = await new ReadNewConversationMessages({findAfterPositionForInquiry}, toConversationMessageDto).execute({inquiryId: "inquiry-1", afterCursor: 1});
  ...
  it("maps missing conversations and persistence details safely", async () => {
    ...
    await expect(new ReadNewConversationMessages({findAfterPositionForInquiry: missing}, toConversationMessageDto).execute({inquiryId: "inquiry-1", afterCursor: -1})).resolves.toEqual({status: "conversation_not_found"});
    ...
    const result = await new ReadNewConversationMessages({findAfterPositionForInquiry: failing}, toConversationMessageDto).execute({inquiryId: "inquiry-1", afterCursor: -1});
    expect(result).toEqual({status: "persistence_failed"});
    expect(JSON.stringify(result)).not.toContain("database secret");
  });

  it("projects provider-neutral Staff updates with authorized actor attribution", async () => {
    const findAfterPositionForInquiry = vi.fn<ConversationMessageUpdateReader["findAfterPositionForInquiry"]>().mockResolvedValue([{position: 2, message}]);
    const result = await new ReadNewConversationMessages({findAfterPositionForInquiry}, toStaffConversationMessageDto).execute({inquiryId: "inquiry-1", afterCursor: 1});

    expect(result).toEqual({status: "found", updates: [{cursor: 2, message: {
      id: "message-2",
      senderType: "INTERNAL_USER",
      channel: "TELEGRAM",
      actorReference: "staff:admin-main",
