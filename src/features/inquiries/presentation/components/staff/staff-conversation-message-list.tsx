...
import {requestedTranslationTarget} from "@/features/conversation-translation/domain/services/translation-scheduling-policy";
  ...
  return (
    <ol role="log" aria-live="polite" aria-relevant="additions" aria-label={labels.messageList} className="space-y-3">
      {messages.map((message) => {
        // Mode changes only affect future scheduling; older unscheduled messages remain requestable.
        const target = message.translation
          ? requestedTranslationTarget({
              senderType: message.senderType,
              sourceLocale: message.translation.sourceLocale,
              customerTargetLocale: message.translation.customerTargetLocale,
              staffTargetLocale: staffWorkingLocale,
            })
          : null;
        const requestLabel = target && !message.translation?.translations.some((translation) => translation.targetLocale === target)
          ? message.senderType === "INTERNAL_USER" ? labels.translation?.translateForCustomer : labels.translation?.translateForStaff
          : null;
        return <li key={message.id} className="min-w-0 rounded-xl border border-stone-200 bg-stone-50 p-4">
          ...
          {labels.translation && message.translation ? <MessageTranslation value={message.translation} labels={labels.translation} outbound={message.senderType !== "CUSTOMER"} canConfirmSource={message.senderType !== "SYSTEM"} customerLanguageResponse={message.senderType === "AI_AGENT"} manualRequestLabel={requestLabel} inquiryId={inquiryId} messageId={message.id} canReply={canReply} onResolved={onTranslationResolved} /> : null}
