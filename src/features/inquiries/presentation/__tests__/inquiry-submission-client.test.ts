...
import {inquiryServerFailure,parseInquirySubmissionResponse,requestInquirySubmissionWithTimeout,revealInquiryFeedback} from "@/features/inquiries/presentation/components/inquiry-form";
...
describe("Inquiry success response contract",()=>{
  it("accepts only the exact 201 closed contract",async()=>expect(await parseInquirySubmissionResponse(json({status:"created",inquiryId:"valid_id-1"}))).toEqual({status:"created",inquiryId:"valid_id-1"}));
  it.each([
    ...
    json(undefined),json({status:"accepted",inquiryId:"id"}),json({status:"created"}),json({status:"created",inquiryId:"bad/id"}),json({status:"created",inquiryId:"id",extra:true}),
...
describe("Inquiry submission timing and indexed errors",()=>{
  ...
  it("maps later Product rows to stable accessible targets",()=>{
    ...
    expect(inquiryServerFailure("items.1.quantity")).toEqual({field:"quantity",code:"invalid",itemIndex:1});
    expect(inquiryServerFailure("items.2.unit")).toEqual({field:"unit",code:"invalid",itemIndex:2});
  });
});
