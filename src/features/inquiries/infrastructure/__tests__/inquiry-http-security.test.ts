...
describe("Inquiry origins", () => {
  ...
  const configureProduction=()=>{vi.stubEnv("NODE_ENV","production");vi.stubEnv("YOLPOL_DEPLOYMENT_ENVIRONMENT","production");vi.stubEnv("YOLPOL_APP_ORIGIN","https://yolpol.com");};
  it("accepts missing, loopback, and approved development LAN origins",()=>{
  ...
  it("accepts only the configured application Origin in production",()=>{configureProduction();expect(accepted("https://yolpol.com","https://internal:3000/api/inquiries")).toBe(true);expect(accepted("https://www.yolpol.com","https://internal:3000/api/inquiries")).toBe(false);});
  ...
  it.each(["http://localhost:3000","http://127.0.0.1:3000","http://192.168.1.100:3000"])("rejects development Origin %s in production",origin=>{configureProduction();expect(accepted(origin,origin.replace("192.168.1.100","localhost")+"/api/inquiries")).toBe(false);});
  ...
  it("does not allow the configured LAN origin in production",()=>{configureProduction();expect(accepted("http://192.168.1.100:3000","http://192.168.1.100:3000/api/inquiries")).toBe(false);});
});
