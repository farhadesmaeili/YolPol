...
describe("TelegramCommunicationAdapter", () => {
  ...
  it("keeps reply parsing inert until the Telegram update contract is introduced", () => {
    const adapter = new TelegramCommunicationAdapter({async findAuthorizedNotificationRecipients() { return []; }}, {async sendMessage() { return; }});
    expect(adapter.toExternalChannelReply({untrusted: true})).toBeNull();
  });
});
