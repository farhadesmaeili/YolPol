...
describe("PostgresInquiryRepository", () => {
  ...
  it("applies committed migrations and exposes the expected tables", async () => {
    await migrate(drizzle(pool, {schema: inquiryPostgresSchema}), {migrationsFolder: resolve("drizzle")});
    const result = await pool.query<{table_name: string}>("select table_name from information_schema.tables where table_schema = 'public' and table_name in ('ai_credential_references','ai_model_profile_capabilities','ai_model_profiles','ai_operation_policy','ai_policy_events','ai_provider_configs','ai_provider_registry_events','ai_provider_runtime_health','ai_schedule_windows','communication_recipients','conversation_access','conversation_ai_control_events','conversation_ai_controls','conversation_ai_response_jobs','conversation_messages','conversations','inquiries','inquiry_assignments','inquiry_items','inquiry_outbox','inquiry_team_members','inquiry_workflow_events','telegram_connection_requests','telegram_inquiry_deliveries','telegram_staff_links') order by table_name");
    expect(result.rows.map(({table_name}) => table_name)).toEqual(["ai_credential_references", "ai_model_profile_capabilities", "ai_model_profiles", "ai_operation_policy", "ai_policy_events", "ai_provider_configs", "ai_provider_registry_events", "ai_provider_runtime_health", "ai_schedule_windows", "communication_recipients", "conversation_access", "conversation_ai_control_events", "conversation_ai_controls", "conversation_ai_response_jobs", "conversation_messages", "conversations", "inquiries", "inquiry_assignments", "inquiry_items", "inquiry_outbox", "inquiry_team_members", "inquiry_workflow_events", "telegram_connection_requests", "telegram_inquiry_deliveries", "telegram_staff_links"]);
