...
import {contactMethods, inquiryUnits} from "@/features/inquiries/domain/types/inquiry-types";
...
export function parseSubmissionPayload(value: unknown): SubmissionPayloadParseResult {
  ...
  keysAllowed(contact, ["fullName", "company", "email", "phone", "telegramUsername", "preferredMethod"], "contact", issues); keysAllowed(location, ["country", "city"], "location", issues); keysAllowed(privacy, ["accepted", "policyVersion"], "privacy", issues); keysAllowed(source, ["locale", "path"], "source", issues);
  const preferredMethod = own(contact, "preferredMethod"); if (preferredMethod === undefined) issues.push({field: "contact.preferredMethod", code: "missing"}); else if (typeof preferredMethod !== "string" || !contactMethods.includes(preferredMethod as never)) issues.push({field: "contact.preferredMethod", code: "invalid_value"});
  ...
  if (itemValues === undefined) issues.push({field: "items", code: "missing"}); else if (!Array.isArray(itemValues)) issues.push({field: "items", code: "invalid_type"}); else itemValues.forEach((candidate, index) => {
    const field = `items.${index}`; if (!plainRecord(candidate)) { issues.push({field, code: "invalid_type"}); return; } keysAllowed(candidate, ["productId", "quantity", "unit"], field, issues);
    const productId = requiredString(candidate, "productId", `${field}.productId`, issues); const quantity = own(candidate, "quantity"); const unit = own(candidate, "unit");
    if (quantity === undefined) issues.push({field: `${field}.quantity`, code: "missing"}); else if (typeof quantity !== "number" || !Number.isFinite(quantity)) issues.push({field: `${field}.quantity`, code: "invalid_type"});
    if (unit === undefined) issues.push({field: `${field}.unit`, code: "missing"}); else if (typeof unit !== "string" || !inquiryUnits.includes(unit as never)) issues.push({field: `${field}.unit`, code: "invalid_value"});
    if (typeof quantity === "number" && Number.isFinite(quantity) && typeof unit === "string" && inquiryUnits.includes(unit as never)) items.push({productId, quantity, unit: unit as SubmitInquiryInput["items"][number]["unit"]});
  });
  const result: SubmitInquiryInput = {contact: {fullName: requiredString(contact, "fullName", "contact.fullName", issues), company: optionalString(contact, "company", "contact.company", issues), email: requiredString(contact, "email", "contact.email", issues), phone: requiredString(contact, "phone", "contact.phone", issues), telegramUsername: optionalString(contact, "telegramUsername", "contact.telegramUsername", issues), preferredMethod: preferredMethod as SubmitInquiryInput["contact"]["preferredMethod"]}, location: {country: requiredString(location, "country", "location.country", issues), city: optionalString(location, "city", "location.city", issues)}, destination, message, privacy: {accepted: accepted as boolean, policyVersion: requiredString(privacy, "policyVersion", "privacy.policyVersion", issues)}, source: {locale: locale as SubmitInquiryInput["source"]["locale"], path: requiredString(source, "path", "source.path", issues)}, items};
