...
export class InquiryTestBuilder {
  ...
  buildReconstituted(value: Partial<Pick<InquiryReconstitutionInput, "status" | "updatedAt">> = {}): Inquiry { return Inquiry.reconstitute({...this.input, status: value.status ?? "NEW", updatedAt: value.updatedAt ?? this.input.createdAt}); }
}
