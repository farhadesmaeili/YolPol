import {DuplicateInquiryIdError, type CatalogProduct, type Clock, type InquiryIdGenerator, type InquiryNotificationDispatcher, type InquiryProductCatalog, type NotificationChannel, type NotificationResult} from "@/features/inquiries/application/ports/inquiry-ports";
...
export class FakeInquiryRepository {
  private readonly records: InquiryReconstitutionInput[] = []; failWith?: Error;
  constructor(private readonly operations: string[] = []) {}
  get saved(): readonly Inquiry[] { return this.records.map(restore); }
  async save(inquiry: Inquiry) { this.operations.push("persist"); if (this.failWith) throw this.failWith; if (this.records.some(({id}) => id === inquiry.id.value)) throw new DuplicateInquiryIdError(); this.records.push(snapshot(inquiry)); }
...
export class FakeInquiryNotificationDispatcher implements InquiryNotificationDispatcher { requested: NotificationChannel[] = []; failures = new Set<NotificationChannel>(); constructor(private readonly operations: string[] = []) {} async dispatch(_inquiry: Inquiry, channel: NotificationChannel): Promise<NotificationResult> { this.operations.push(`notify:${channel}`); this.requested.push(channel); return {status: this.failures.has(channel) ? "failed" : "requested"}; } }
