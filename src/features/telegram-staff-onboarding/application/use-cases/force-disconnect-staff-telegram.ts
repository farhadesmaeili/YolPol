...
export class ForceDisconnectStaffTelegram {
  ...
  async execute(input: Readonly<{principal: StaffPrincipal; targetStaffAccountId: string}>): Promise<Readonly<{status: "disconnected" | "unavailable"}>> {
    try {
      const status = await this.repository.forceDisconnect({
