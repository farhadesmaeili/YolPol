...
export class RevokeOwnTelegramConnectionRequest {
  ...
  async execute(input: Readonly<{principal: StaffPrincipal}>): Promise<Readonly<{status: "revoked" | "unavailable"}>> {
    try {
      const status = await this.repository.revokeOwnRequest({
