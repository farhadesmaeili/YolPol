...
export const conversationAiResponseJobs = pgTable("conversation_ai_response_jobs", {
  ...
  agentDecision: varchar("agent_decision", {length: 16}),
  escalationReason: varchar("escalation_reason", {length: 64}),
  ...
  check("conversation_ai_response_jobs_decision_check", sql`(${table.status} = 'SUCCEEDED' and ${table.agentDecision} is not null and ${table.agentDecision} in ('RESPOND','ESCALATE')) or (${table.status} <> 'SUCCEEDED' and ${table.agentDecision} is null)`),
  check("conversation_ai_response_jobs_escalation_check", sql`(${table.agentDecision} is not distinct from 'ESCALATE' and ${table.escalationReason} is not null and ${table.escalationReason} in ('PRICE_QUOTATION','DISCOUNT_REQUEST','PAYMENT_TERMS','CREDIT_REQUEST','CONTRACT_COMMITMENT','LEGAL_OR_REGULATORY_GUARANTEE','CUSTOMS_GUARANTEE','DELIVERY_COMMITMENT','AVAILABILITY_UNVERIFIED','PRODUCT_FACT_UNAVAILABLE','CONTRADICTORY_TRUSTED_DATA','UNSUPPORTED_REQUEST','LOW_CONFIDENCE','INVALID_TOOL_CALL','UNKNOWN_TOOL','LOOP_LIMIT_REACHED','EXECUTION_DEADLINE_REACHED')) or (${table.agentDecision} is distinct from 'ESCALATE' and ${table.escalationReason} is null)`),
