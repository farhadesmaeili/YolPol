import type {ChangeGlobalTranslationDefaultsInput} from "@/features/conversation-translation/application/dto/translation-control-dto";
...
export class ChangeGlobalTranslationDefaults {
  constructor(
  ...
  async execute(input: ChangeGlobalTranslationDefaultsInput & Readonly<{principal: StaffPrincipal}>) {
    if (!this.authorization.mayManageTranslationSettings(input.principal)) return {status: "forbidden" as const};
    ...
    try {
      const result = await this.repository.changeGlobalDefaults({
