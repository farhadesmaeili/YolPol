...
export type TranslationLabels = Readonly<{
  original: string; translation: string; pending: string; failed: string; ready: string; sameLanguage: string; unknown: string; authoring: string; authoringManual: string;
  ...
  translate: string; translateForStaff: string; translateForCustomer: string; customerLanguageResponse: string;
  languages: Readonly<Record<Locale, string>>;
}>;

export function MessageTranslation({value, labels, outbound = Boolean(value.customerTargetLocale), canConfirmSource = true, customerLanguageResponse = false, manualRequestLabel, inquiryId, messageId, canReply, onResolved}: Readonly<{
  value: MessageTranslationView; labels: TranslationLabels; outbound?: boolean; canConfirmSource?: boolean; customerLanguageResponse?: boolean; manualRequestLabel?: string | null; inquiryId?: string; messageId?: string; canReply?: boolean; onResolved?: () => void;
}>) {
  ...
  return <div className="mt-3 space-y-2 text-sm" aria-live="polite">
    ...
    {!skipped && value.customerTargetLocale && value.sourceLocale === value.customerTargetLocale ? <p className="text-xs">{customerLanguageResponse ? labels.customerLanguageResponse : labels.sameLanguage}</p> : null}
    ...
    {canReply && inquiryId && messageId && !skipped ? <TranslationRemediationActions key={value.version} inquiryId={inquiryId} messageId={messageId} value={value} outbound={outbound} canConfirmSource={canConfirmSource} manualRequestLabel={manualRequestLabel} labels={labels} onResolved={onResolved} /> : null}
  </div>;
}
