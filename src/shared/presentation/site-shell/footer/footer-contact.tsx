...
export function FooterContact({ labels, isRtl }: { labels: FooterContactLabels; isRtl: boolean }) {
  return (
    <section className="min-w-0 text-start">
      ...
      <div className="relative mt-8 overflow-hidden border border-stone-950/[0.09] bg-white/30 p-5 backdrop-blur-xl sm:p-6">
        ...
        <address className="min-w-0 space-y-2 text-sm not-italic">
          ...
          <a href={siteConfig.contact.phoneHref} className={contactLinkClass}>
          ...
          <div aria-hidden="true" className="h-px bg-stone-950/[0.07]" />
          <a href={siteConfig.contact.whatsappHref} target="_blank" rel="noopener noreferrer" className={contactLinkClass}>
            <span>{labels.whatsapp}</span>
            <LtrIsolate className="shrink-0">{siteConfig.contact.whatsapp}</LtrIsolate>
          </a>
          <div aria-hidden="true" className="h-px bg-stone-950/[0.07]" />
          <div className="py-2">
            <p className="text-xs font-medium text-stone-400">{labels.location}</p>
            <p className="mt-2 max-w-xs break-words leading-6 text-stone-700">{siteConfig.contact.location}</p>
          </div>
        </address>
      </div>
