...
export function HomeHeroContent({ model }: { model: HomeHeroViewModel }) {
  return (
    <div dir={model.isRtl ? "rtl" : "ltr"} className={`relative z-30 py-6 text-start lg:py-14 ${model.isRtl ? "lg:order-2 lg:ps-8" : "lg:order-1 lg:pe-8"}`}>
      ...
      <div aria-hidden="true" dir="ltr" className={`mt-8 flex max-w-md items-center gap-4 ${model.isRtl ? "me-auto" : ""}`}>
      ...
      <div className={`mt-8 max-w-2xl border-s border-stone-950/15 ps-5 ${model.isRtl ? "me-auto" : ""}`}>
        <p className="max-w-xl text-base leading-8 text-stone-600 sm:text-lg sm:leading-9">{model.description}</p>
        <p className="mt-4 max-w-xl text-sm font-medium leading-7 text-stone-700 sm:text-base">{model.catalog}</p>
      </div>

      <div className="mt-9 flex flex-wrap justify-start gap-3 sm:mt-11 sm:gap-4">
        <Link href="/inquiry" className="group inline-flex min-h-14 max-w-full items-stretch overflow-hidden border border-emerald-950/20 bg-emerald-950 text-white shadow-[0_20px_55px_-32px_rgba(6,78,59,0.65)] outline-none transition-[background-color,box-shadow] duration-500 hover:bg-emerald-900 focus-visible:ring-2 focus-visible:ring-emerald-700 focus-visible:ring-offset-4 motion-reduce:transition-none sm:min-h-16">
          <span className="flex min-w-0 items-center px-5 text-sm font-semibold sm:px-8">{model.inquiryCta}</span>
